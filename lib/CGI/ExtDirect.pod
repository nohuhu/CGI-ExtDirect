...
Unlike prior versions, CGI::ExtDirect 3.0+ constructor does not accept
individual configuration options itself. You will need to create an
instance of L<RPC::ExtDirect::Config> with all required options set,
and pass it to CGI::ExtDirect L<constructor|/new> to be used.

Refer to L<RPC::ExtDirect::Config/OPTIONS> for the list of options
and their values.
...
L<CGI::ExtDirect> provides several public methods:
...
Note that the examples do not require CGI::ExtDirect to be installed
